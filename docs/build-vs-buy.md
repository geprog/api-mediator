...
| S12 | Per-record sequential ordering **across both directions of a pair** — one queue per `RecordLink` (deliberately *not* per `(mapping, resourceId)`), identity-value keying before a link exists, continuation handoff at link creation | *Ordering and consistency* |
...
- **S1 (✓), S11 (◐), S12 (○):** schedule triggers per workflow are native; per-node retry settings, error workflows, and retry-from-UI of stored executions approximate retry/dead-letter/replay, but parking semantics ("superseded by any later successful sync") are custom; per-key ordering doesn't exist — per-workflow concurrency 1 serializes each *rule*, but S12 requires serializing the two directions of a pair (two workflows) over one shared `RecordLink` queue, including the pre-link identity-value keying and the handoff at link creation, and that cross-workflow ordering has no n8n counterpart.
...
- **S1 (✓), S11 (✓), S12 (◐):** timers/schedulers, redelivery policies, and dead-letter channel are native EIPs; per-`RecordLink` ordering — shared across the pair's two routes, keyed pre-link by identity value — needs explicit route design around a mediator-computed key (a resequencer helps with ordering, not with cross-route mutual exclusion). **S2 (◐):** cursor storage in your own repository. **S3–S9 (○):** the sync core is absent, exactly as [related-work.md](related-work.md)'s "overlaps only with the execution layer" says.
...
- **S1 (✓), S2 (✓), S11 (✓):** operational rows the workflow tools fumble are Temporal's core product: [Schedules](https://docs.temporal.io/schedules) drive per-rule polling; workflow state is durable by construction (cursors survive anything); retry policies with backoff, unbounded durable retries, and human-in-the-loop intervention are native. **S12 (◐):** entity workflows give native per-key serialization, but the key S12 requires is the `RecordLink` — one workflow shared by **both directions'** rules, not a workflow per `(rule, record)`, which would re-open exactly the cross-direction race the keying exists to prevent (see *Ordering and consistency* in [sync-engine.md](architecture/sync-engine.md)) — and the pre-link identity-value fallback with its continuation handoff at link creation is mediator logic on top: re-keying a record's queue mid-stream needs an explicit drain/signal pattern Temporal doesn't provide out of the box. **S9 (◐):** backfill as a long-running, resumable, progress-queryable workflow is a natural fit; its semantics (link-only vs push, baseline seeding) stay custom. **S10 (◐):** deterministic replay preserves computed idempotency keys across crashes and retries; key derivation and `SyncEvent` lookback stay custom.
...
- **What generic workflow tools do convert (S1, parts of S11, X2) is the cheap 20%** — scheduling, HTTP execution, credentials — available in any stack's libraries; S12's cross-direction per-`RecordLink` ordering, with its identity-value fallback and link-creation handoff, is beyond all of them and only partially covered even by Temporal. n8n and Node-RED, the two candidates the evaluation started from, convert the least differentiating rows while adding a license contingency (n8n) or a sandboxing conflict (Node-RED).
