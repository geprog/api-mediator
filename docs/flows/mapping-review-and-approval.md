...
9. Depending on the spec-pair roles:
   - If both apps are `PROVIDER`-role peers, the Sync Engine instantiates a `SyncRule` **per mapped resource pair** for this one direction — each created disabled until its identity key is confirmed (step 6) and its initial backfill has run or been explicitly skipped (see [architecture/sync-engine.md](../architecture/sync-engine.md)) — see [sync-webhook-push.md](sync-webhook-push.md) / [sync-polling-pull.md](sync-polling-pull.md). If a counterpart mapping exists (or is approved later), its own `SyncRule`s are a separate instantiation — two one-way rules per resource pair, not one bidirectional rule.
