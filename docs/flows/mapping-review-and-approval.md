...
2. The Approval Service returns `MappingProposalItem`s sorted by ascending confidence / descending ambiguity — items flagged `reviewRequired` (confidence below threshold) surface first. Alongside the items, the view lists the source resources for which stage 1 shortlisted **no** counterpart (`MappingProposal.shortlistResult`, see [architecture/mapping-engine.md](../architecture/mapping-engine.md)), each with a manual "analyze this resource pair anyway" action that triggers a scoped detail analysis — the escape hatch for a shortlist miss. Resources the operator excluded from analysis (`ApiSpec.analysisExclusions`) are listed separately *as excluded* — distinct from shortlist misses, and without the "analyze anyway" action: analyzing one means removing its exclusion, a deliberate scope edit rather than a review-screen override (see *Scoping down* in [architecture/mapping-engine.md](../architecture/mapping-engine.md)).
...
5. On the approve action, the Approval Service validates the edited paths against the target spec's IR, then assembles the accepted items under a new (or updated) `ApprovedMapping`: `kind = field` items become `FieldMapping`s (keeping their request/response `phase` on a consumer-provider proposal), `kind = operation` items become `OperationMapping`s — each classified with an `action` (create/read/update/delete) from the target operation's IR, correctable by the reviewer where the heuristic gets it wrong — and `kind = parameter` items (consumer-provider only) become `ParameterMapping`s under their operation pairing (see [architecture/data-model.md](../architecture/data-model.md)). Since a `MappingProposal` is always one-directional (`sourceSpecId → targetSpecId`), the resulting `ApprovedMapping` is always one-directional too — there is no separate "approve as bidirectional" action. On a consumer-provider proposal, the request- and response-phase items are reviewed in this same pass and land together in the one resulting `ApprovedMapping` — a request phase without its response phase serves nothing (see [architecture/data-model.md](../architecture/data-model.md)).
6. For a **peer-peer** proposal, the reviewer confirms the **identity key**: exactly one `FieldMapping` per mapped resource pair gets `isIdentityKey = true`, pre-selected from the LLM's `identityCandidate` suggestion but never auto-confirmed (see [architecture/mapping-engine.md](../architecture/mapping-engine.md)) — along with, where the target's collection read offers one, the lookup parameter identity matching will query by (`targetLookupParamRef`, see [architecture/data-model.md](../architecture/data-model.md)). The identity pairing is shared with the reverse direction and must be value-preserving: when the counterpart mapping's resource pair already has a confirmed identity key, the same field pairing is pre-locked here — confirming a different one is a validation error — and only a `rename` transform is accepted on an identity `FieldMapping` (see [architecture/data-model.md](../architecture/data-model.md)). Approval can complete without this confirmation, but a resource pair's `SyncRule` cannot be *enabled* until its identity key exists — record correlation is impossible without it (see *Identity correlation* in [architecture/sync-engine.md](../architecture/sync-engine.md)).
