...
- Enabling a `SyncRule` triggers its one-time initial backfill; the rule does not start polling until that backfill has completed or been explicitly skipped as part of the enable action — see *Initial backfill* in [architecture/sync-engine.md](../architecture/sync-engine.md).
