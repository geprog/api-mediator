...
3. The Request Router matches the operation to its `AdapterEndpoint`, and the request is validated against the consumer operation's request schema and parameters — violations (including a union filter parameter with neither pushdown nor post-merge configuration, or a union sort/pagination parameter without configured post-merge semantics) are rejected before any backend is involved (see [architecture/adapter-engine.md](../architecture/adapter-engine.md)).
4. The Resolution Planner loads the endpoint's persisted `AdapterBinding`(s) (primary/fallback/supplement) and their `ApprovedMapping`s, re-validating that each mapping is still `active` — a `stale` binding fails as a distinct `mapping-stale` error, a `suspended` one as `mapping-suspended`, rather than a generic upstream error (see [architecture/adapter-engine.md](../architecture/adapter-engine.md)).
...
6. The Outbound Call Executor calls the backend app(s) grouped by `executionOrder` — bindings sharing an order run in parallel; a binding with `dependsOnBindingId` set runs only once that binding's response is available and receives it as input — the upstream's response-phase transform runs as soon as its response arrives, since `chainInputs` are defined over the consumer-shape response.
