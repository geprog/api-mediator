...
4. `composition-required` endpoints surface in the UI (and as a Grafana alert, see [architecture/observability.md](../architecture/observability.md)) for an explicit composition decision. The composer chooses:
   ...
   - `executionOrder` / `dependsOnBindingId` for parallel, sequential, or chained execution — strategy-scoped, see the validity rules in [architecture/adapter-engine.md](../architecture/adapter-engine.md) — including, for a chained binding, which upstream consumer-shape response fields feed its backend inputs (the binding's `chainInputs`, see [architecture/data-model.md](../architecture/data-model.md)),
   - for `collection-union`: an optional **dedup key** (a consumer-schema field) where cross-backend `RecordLink`s aren't available to collapse duplicates — link-based dedup itself is offered only when every contributing backend resource has a confirmed `ResourceBinding.nativeIdRef`, since it needs each row's backend-native id as provenance — **post-merge filter semantics** (`postMergeFilters`: constrained consumer-schema field + comparison operator) for each consumer filter parameter not mapped in every binding, and **sort/pagination semantics** (`postMergeSorts`, `postMergePagination` — heuristically pre-filled, composer-confirmed), since sort and pagination are never pushed down; a request using any of these parameters without configured semantics is rejected rather than answered unfiltered, unsorted, or mispaged (see *Aggregation strategies* in [architecture/adapter-engine.md](../architecture/adapter-engine.md)),
...
5. The Adapter Engine validates the composition — role-validity table, the `executionOrder`/`dependsOnBindingId` rules (including no order ties under `fanout-first-success`), chained bindings' `chainInputs` referencing fields the upstream binding's consumer-shape response actually provides, `postMergeFilters`/`postMergeSorts`/`postMergePagination` referencing real consumer parameters and consumer-schema fields, and write operations must remain `single` (see [architecture/adapter-engine.md](../architecture/adapter-engine.md)) — then activates it: `proposed` bindings become `active` and the endpoint returns to `active`. This same validation is re-run against a successor mapping's content at adoption (see *Successor adoption* in [architecture/extensibility.md](../architecture/extensibility.md)).
