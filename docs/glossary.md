...
- **ResourceBinding** — the confirmed per-resource operational bindings of an `ApiSpec` — native-id field, pagination parameters, delta cursor, change-timestamp field — that make the app's declared `capabilities` executable; derived heuristically at ingestion, confirmed or corrected by the operator before sync rules over the resource can be enabled.
