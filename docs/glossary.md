...
- **ScopeLink** — the persisted correspondence between a **container** (scope) in app A and the same logical container in app B — the container-level analog of `RecordLink` (a Gitea repo ↔ a Vikunja project). Established by `constant`, `identity-match` (a discovered match of the two sides' *scope identity keys*), or `manual`; it resolves each record's target scope so a scoped operation's non-record-id path parameters can be filled, and it scopes record-level identity matching (a record matches *within* its linked container, not globally). Deliberately not an `ApprovedMapping` — a container correspondence is identity state, not a reviewed data-sync mapping.
- **scope identity key** — the confirmed field(s) whose values identify the same **container** on both sides (Gitea repo `full_name` ↔ Vikunja project `title`/`identifier`); operator-confirmed like a record identity key but over the container resource, and how an `identity-match` `ScopeLink` is established. A `SyncRule` enablement precondition whenever any of its scope bindings is `kind: scope-link`.
