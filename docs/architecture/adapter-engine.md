...
6. Outbound Call Executor calls the backend app(s) grouped by `executionOrder` — bindings sharing an `executionOrder` run in parallel; a binding with `dependsOnBindingId` set runs only after that binding's response is available, its `chainInputs` filling the dependent backend call's parameters from that upstream consumer-shape response.
