...
- **Adapter engine** — request rate/latency/error rate per `AdapterEndpoint`, cache hit rate, partial-failure/degraded-response rate.
- **Mapping engine** — LLM call latency/error rate/token usage per provider and stage, shortlist yield, escape-hatch usage, analysis-scope exclusions and re-inclusions, proposals pending review, average confidence score trend.
