...
- `sourceParamRef` (a consumer operation parameter — or, on a binding with `dependsOnBindingId`, a field path into the upstream binding's *consumer-shape* response: the chaining input, configured at composition, see [adapter-engine.md](adapter-engine.md)), `targetParamRef` (backend operation parameter)
