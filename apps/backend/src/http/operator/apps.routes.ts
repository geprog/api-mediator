 ...
 * - `POST /api/apps` — register an app + its role-tagged specs (atomic).
 * - `GET /api/apps` — list all apps (unpaginated at Phase-1 scale, OQ7).
...
export function registerAppRoutes(app: FastifyInstance, deps: OperatorApiDeps): void {
  app.post("/api/apps", async (request, reply): Promise<RegisterAppResponse> => {
  ...
  app.get("/api/apps", async (): Promise<AppListResponse> => {
  ...
  app.get("/api/apps/:id/specs", async (request): Promise<AppSpecsResponse> => {
