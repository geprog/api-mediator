...
export function registerSpecRoutes(app: FastifyInstance, deps: OperatorApiDeps): void {
  app.get("/api/specs/:id/ir", async (request): Promise<IrResponse> => {
  ...
  app.post("/api/specs/preview", async (request): Promise<PreviewParseResponse> => {
