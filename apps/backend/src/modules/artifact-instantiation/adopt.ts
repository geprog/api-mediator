 ...
 *
 * **`SyncFieldState` field-pair reconciliation (SL-8.4 archive dropped / SL-8.5 seed added)
 * is deliberately NOT performed in this synchronous, pure-database transaction** and is left
 * as a scoped follow-up, because neither fits it safely: (1) `SyncFieldState` is shared by
 * **both** peer directions over a resource pair, so archiving a field the successor dropped
 * is only correct once the reverse direction's surviving mapping is consulted (else a
 * still-live baseline is wrongly archived); and (2) seeding an added field pair's baseline
 * requires the field's **observed value** — `SyncFieldState.observedHash`/`observedAt` are
 * required and cannot be synthesized without reading the record, i.e. it needs the same async
 * link-only backfill pass (BE-4) that seeds baselines over existing `RecordLink`s. Both are
 * the reconciliation half a later pass performs against the (adopted) successor content; the
 * adoption above establishes the successor as the live relationship those passes reconcile.
 */
 ...
 * the just-approved `ApprovedMapping` carrying a `predecessorMappingId`; `tx` is the
...
export async function adoptSuccessor<TTx>(params: {
  ...
  const { successor, predecessorMappingId, deps, tx } = params;
